...
def test():
    return {"passwords": ["Password1", "Password2", "Password3", "Password4"]}

def password_generator_main(answer_one, answer_two, answer_three, password_length, have_symbols, have_numbers,
                            have_lowercase, have_uppercase, have_ambiguous_characters):
    ...
    is_input_valid = input_validation(answer_one, answer_two, answer_three, password_length, have_symbols,
                                      have_numbers, have_lowercase, have_uppercase, have_ambiguous_characters)
    ...
    if is_input_valid != 0:
        ...
        if is_input_valid == 1:
            ...
            print("ERROR: You must select at least one of 'include lowercase' or 'include uppercase'.")
    ...
    formatted_answers = answer_formatter(answer_one, answer_two, answer_three, have_lowercase, have_uppercase)
...
def input_validation(answer_one, answer_two, answer_three, password_length, have_symbols, have_numbers,
    ...
    if password_length < 1:
        ...
        error_code = 2
    elif have_lowercase == False and have_uppercase == False:
        error_code = 3
...
def answer_formatter(answer_one, answer_two, answer_three, have_lowercase, have_uppercase):
    ...
    for answer in not_formatted_answers:
        if have_lowercase and have_uppercase:
...
def add_numbers_and_symbols(answers_to_add, num_characters_to_add):
    ...
    elif num_characters_to_add != 0 and have_symbols == False and have_numbers == False: # TODO: no access to have_symbols amd have_numbers
        error_code = 1
        password = "".join(answers_to_add)
    else:
        character_pool = []
...
if __name__ == "__main__":
    ...
    for i in range(10):
        generated_password = password_generator_main("pear", "Red Sox", "car", 12,
